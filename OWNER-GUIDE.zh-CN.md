...
仓库里提供了一份可反复复制的[中文文章草稿母版](./templates/中文文章草稿模板.md)。当前电脑还准备了一份可直接填写的私有工作稿：`.drafts-private/下一篇文章.md`。私有工作稿会被 Git 忽略，不会推送到公开 GitHub。

每次发文只需要四步：

1. 打开 `.drafts-private/下一篇文章.md`，只填写中文正文、参考资料和你在意的翻译要求。
...
```text
请读取 `.drafts-private/下一篇文章.md`，按其中的中文内容和资料来源生成一篇完整双语博客文章。
中文是唯一内容来源；先指出事实缺口、歧义、来源不足或隐私风险，不得自行增加事实、经历、成果或立场。
请核实参考资料，把资料编号转换成 BibTeX 与 APA 作者—年份引用；忠实翻译成完整英文，并生成 meta.yaml、en.mdx、zh.mdx。
运行完整内容校验和构建，再打开本地预览供我检查。
本轮不要提交、不要推送、不要部署。
```
